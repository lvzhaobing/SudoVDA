...
static constexpr DWORD IDD_SAMPLE_MONITOR_COUNT = 10; // If monitor count > ARRAYSIZE(s_SampleMonitors), we create edid-less monitors
...
//     {
// 		{
// 			0x00, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0x00, 0x4d, 0xab, 0xcd, 0xab, 0xcd, 0x2b, 0xbc, 0x1a,
// 			0xff, 0x22, 0x01, 0x04, 0xb5, 0x32, 0x1f, 0x78, 0x1f, 0xee, 0x95, 0xa3, 0x54, 0x4c, 0x99, 0x26,
// 			0x0f, 0x50, 0x54, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
// 			0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x02, 0x3a, 0x80, 0x18, 0x71, 0x38, 0x2d, 0x40, 0x58, 0x2c,
// 			0x45, 0x00, 0x63, 0xc8, 0x10, 0x00, 0x00, 0x06, 0x00, 0x00, 0x00, 0xfd, 0x00, 0x17, 0xf0, 0x0f,
// 			0xff, 0x37, 0x00, 0x0a, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x00, 0x00, 0x00, 0xff, 0x00, 0x31,
// 			0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x41, 0x42, 0x43, 0x44, 0x00, 0x00, 0x00, 0xfc,
// 			0x00, 0x53, 0x75, 0x64, 0x6f, 0x56, 0x44, 0x41, 0x20, 0x44, 0x49, 0x53, 0x50, 0x0a, 0x01, 0x03,
// 		},
// 		{
//             { 3840, 2160,  120 },
//             { 1600,  900,  120 },
...
void IndirectDeviceContext::CreateMonitor() {
    ...
    // 	// MonitorInfo.MonitorDescription.pData = const_cast<BYTE*>(edid_base);
    // }
    std::string dispName = "SudoVDD #";
    dispName += std::to_string(connectedDisplayCount + 1);
    MonitorInfo.MonitorDescription.DataSize = sizeof(edid_base);
   	MonitorInfo.MonitorDescription.pData = generate_edid(0xAA55BB66, dispName.c_str());
...
NTSTATUS IddSampleParseMonitorDescription(const IDARG_IN_PARSEMONITORDESCRIPTION* pInArgs, IDARG_OUT_PARSEMONITORDESCRIPTION* pOutArgs)
{
    ...
    pOutArgs->MonitorModeBufferOutputCount = std::size(s_SampleDefaultModes);

    if (pInArgs->MonitorModeBufferInputCount < std::size(s_SampleDefaultModes))
    {
    ...
    else
    {
        ...
        //     {
                ...
                for (DWORD ModeIndex = 0; ModeIndex < std::size(s_SampleDefaultModes); ModeIndex++)
                {
...
NTSTATUS IddSampleMonitorQueryModes(IDDCX_MONITOR MonitorObject, const IDARG_IN_QUERYTARGETMODES* pInArgs, IDARG_OUT_QUERYTARGETMODES* pOutArgs)
{
    ...
    for (size_t i = 0; i < std::size(s_SampleDefaultModes); i++) {
